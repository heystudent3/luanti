...
struct MediaInfo
{
    ...
    MediaInfo(std::string_view path_ = "",
              std::string_view sha256_digest_ = ""):
        path(path_),
        sha256_digest(sha256_digest_),
        no_announce(false),
...
public:
    ...
    */
    ...
    // map key = binary sha256, map value = file path
...
private:
    ...
    */
    ...
    // Rate limiting for mod channel messages (peer_id -> last_message_time)
    std::unordered_map<session_t, float> m_modchannel_rate_limiter;
