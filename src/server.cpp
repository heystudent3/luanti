...
void Server::init()
{
    ...
    // Read Textures and calculate sha256 sums
    fillMediaCache();
    ...
    std::string *digest_to)
{
    ...
    std::string sha256 = hashing::sha256(filedata);
    std::string sha256_hex = hex_encode(sha256);
    if (digest_to)
        *digest_to = sha256;

    // Put in list
    m_media[filename] = MediaInfo(filepath, sha256);
    verbosestream << "Server: " << sha256_hex << " is " << filename
            << std::endl;
...
void Server::sendMediaAnnouncement(session_t peer_id, const std::string &lang_code)
{
    ...
    if (client->net_proto_version < 48) {
        ...
        for (const auto &i : m_media) {
            if (include(i.first, i.second))
                pkt << i.first << base64_encode(i.second.sha256_digest);
        }
    } else {
        ...
        for (const auto &i : m_media) {
            if (include(i.first, i.second)) {
                assert(i.second.sha256_digest.size() == 32);
                pkt.putRawString(i.second.sha256_digest);
            }
        }
    }
...
void Server::DeleteClient(session_t peer_id, ClientDeletionReason reason)
{
    std::wstring message;
    {
        ...
        */
        ...
        // Clear rate limiting data for disconnected peer
        m_modchannel_rate_limiter.erase(peer_id);
...
std::unordered_map<std::string, std::string> Server::getMediaList()
{
    ...
    for (auto &it : m_media) {
        if (it.second.no_announce)
            continue;
        ret.emplace(it.second.sha256_digest, it.second.path);
    }
    return ret;
}
