        ...
        IGameDef *gamedef)
{
    ...
    for (auto &item : input.items) {
        ...
        for (auto j = pairs.begin(); j != pairs.end(); /* No increment here */) {
            if (inputItemMatchesRecipe(item.name, j->first, gamedef->idef())) {
                ...
                j = pairs.erase(j); // Correctly update iterator after erase
                break; // Exit inner loop once replacement is found
            } else {
                ++j; // Increment iterator only if no erase happened
            }
        }
