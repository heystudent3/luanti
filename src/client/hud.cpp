...
bool Hud::handleMouseEvent(const irr::SEvent::SMouseInput &mouse_event)
{
    // Get mouse position relative to window
    v2s32 mouse_pos(mouse_event.X, mouse_event.Y);

    for (size_t i = 0; i != player->maxHudId(); i++) {
        HudElement *e = player->getHud(i);
        if (!e || e->type != HUD_ELEM_SHADER_BUTTON)
            continue;
        ...
        rect += v2s32(floor(e->pos.X * (float) m_screensize.X + 0.5),
                      floor(e->pos.Y * (float) m_screensize.Y + 0.5))
              + offset
              + v2s32(e->offset.X * m_scale_factor,
                       e->offset.Y * m_scale_factor);

        // Handle mouse events for the shader button
        switch (mouse_event.Event) {
            case EMIE_LMOUSE_PRESSED_DOWN: {
                if (rect.isPointInside(mouse_pos)) {
                    if (e->is_menu_open) {
                        s32 menu_y_offset = rect.LowerRightCorner.Y + 5;
                        gui::IGUIFont *const font = g_fontengine->getFont();

                        for (const std::string& shader_name : e->shader_list) {
                            EnrichedString shader_text(unescape_string(utf8_to_wide(shader_name)));
                            core::dimension2d<u32> textsize = font->getDimension(shader_text.c_str());
                            core::rect<s32> shader_rect(rect.UpperLeftCorner.X, menu_y_offset,
                                                        rect.UpperLeftCorner.X + textsize.Width, menu_y_offset + textsize.Height);

                            if (shader_rect.isPointInside(mouse_pos)) {
                                e->current_shader = shader_name;
                                // Apply the shader (assuming client->getShaderSource() and setShader are available)
                                client->getShaderSource()->setShader(e->current_shader);
                                e->is_menu_open = false;
                                return true;
                            }
                            menu_y_offset += textsize.Height + 2;
                        }
                        // Clicked on button area but not on a shader name, close menu
                        e->is_menu_open = false;
                        return true;
                    } else {
                        // Clicked on button, start dragging or open menu
                        m_dragged_hud_element = e;
                        m_drag_start_mouse_pos = mouse_pos;
                        m_drag_start_offset = e->offset;
                        e->is_dragging = true;
                        e->is_menu_open = true; // Open menu on first click
                        return true;
                    }
                } else if (e->is_menu_open) {
                    // Clicked outside button and menu, close menu
                    e->is_menu_open = false;
                    return true;
                }
                break;
            ...
            case EMIE_MOUSE_MOVED: {
                if (m_dragged_hud_element == e && e->is_dragging) {
                    v2s32 delta = mouse_pos - m_drag_start_mouse_pos;
                    e->offset.X = m_drag_start_offset.X + (float)delta.X / m_scale_factor;
                    e->offset.Y = m_drag_start_offset.Y + (float)delta.Y / m_scale_factor;
                    return true;
                }
                break;
            }
            ...
            case HUD_ELEM_WIDGET: {
                ...
                if (!e->name.empty()) {
                    ...
                    v2s32 text_offs(rect.UpperLeftCorner.X + (rect.getWidth() - textsize.Width) / 2,
                                                rect.UpperLeftCorner.Y + rect.getHeight() / 2 - textsize.Height / 2);
                                        font->draw(text_widget.c_str(), text_offs, text_color);
                                }
                                break;
                        }
                        case HUD_ELEM_SHADER_BUTTON: {
                                video::ITexture *texture = tsrc->getTexture(e->text);
                                if (!texture) {
                                        warningstream << "HUD: Unknown shader button texture: " << e->text << std::endl;
                                        continue;
                                }
                                ...
                                if (e->is_menu_open) {
                                        s32 menu_y_offset = rect.LowerRightCorner.Y + 5;
                                        for (const std::string& shader_name : e->shader_list) {
                                                video::SColor text_color = (shader_name == e->current_shader) ?
                                                        video::SColor(255, 255, 255, 0) : // Highlight selected shader (yellow)
                                                        video::SColor(255, 255, 255, 255); // Default color (white)
                                                
                                                EnrichedString shader_text(unescape_string(utf8_to_wide(shader_name)), text_color);
                                                core::dimension2d<u32> textsize = font->getDimension(shader_text.c_str());
                                                
                                                v2s32 text_pos(rect.UpperLeftCorner.X, menu_y_offset);
                                                font->draw(shader_text.c_str(), text_pos, text_color);
                                                
                                                menu_y_offset += textsize.Height + 2; // Add some padding
                                        }
                                }
                                break;
                        }
