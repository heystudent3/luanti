...
bool clientMediaUpdateCache(const std::string &raw_hash, const std::string &filedata)
{
    ...
    std::string hash_hex = hex_encode(raw_hash);
    if (!media_cache.exists(hash_hex))
        return media_cache.update(hash_hex, filedata);
    return false;
}
...
bool clientMediaUpdateCacheCopy(const std::string &raw_hash, const std::string &path)
{
    ...
    std::string hash_hex = hex_encode(raw_hash);
    if (!media_cache.exists(hash_hex))
        return media_cache.updateCopyFile(hash_hex, path);
    return false;
}
...
void ClientMediaDownloader::addFile(const std::string &name, const std::string &sha256)
{
    ...
    // length of sha256 must be exactly 32 (256 bits), else ignore the file
    if (sha256.size() != 32) {
        errorstream << "Client: ignoring illegal SHA256 sent by server: "
                << hex_encode(sha256) << " \"" << name << "\""
                << std::endl;
        return;
    }
...
void ClientMediaDownloader::initialStep(Client *client)
{
    ...
    for (auto &file_it : m_files) {
        ...
        const std::string &sha256 = filestatus->sha256;

        if (tryLoadFromCache(name, sha256, client)) {
    ...
    if (!USE_CURL || m_uncached_count == 0 || m_remotes.empty()) {
        ...
        // Otherwise start off by requesting each server's sha256 set
        ...
        const HTTPFetchResult &fetch_result)
{
    ...
    if (fetch_result.succeeded) {
        try {
            ...
            std::set<std::string> sha256_set; // Changed from sha1_set to sha256_set
            deSerializeHashSet(fetch_result.data, sha256_set);
            ...
            for(auto it = m_files.upper_bound(m_name_bound);
                ...
                if (!f->received && sha256_set.count(f->sha256))
        ...
        Client *client)
{
    ...
    if (fetch_result.succeeded) {
        bool success = checkAndLoad(name, filestatus->sha256,
        ...
        Client *client)
{
    ...
    // If so, load it
    checkAndLoad(name, filestatus->sha256, data, false, client);
...
bool IClientMediaDownloader::tryLoadFromCache(const std::string &name,
    const std::string &sha256, Client *client)
{
    std::ostringstream tmp_os(std::ios_base::binary);
    bool found_in_cache = m_media_cache.load(hex_encode(sha256), tmp_os);

    // If found in cache, try to load it from there
    if (found_in_cache)
        return checkAndLoad(name, sha256, tmp_os.str(), true, client);

    return false;
}

bool IClientMediaDownloader::checkAndLoad(
        const std::string &name, const std::string &sha256,
        const std::string &data, bool is_from_cache, Client *client)
{
    ...
    std::string sha256_hex = hex_encode(sha256);

    // Compute actual checksum of data
    std::string data_sha256 = hashing::sha256(data);

    // Check that received file matches announced checksum
    if (data_sha256 != sha256) {
        std::string data_sha256_hex = hex_encode(data_sha256);
        infostream << "Client: "
            << cached_or_received_uc << " media file "
            << sha256_hex << " \"" << name << "\" "
            << "mismatches actual checksum " << data_sha256_hex
            << std::endl;
        return false;
    }
    ...
    if (!is_from_cache && m_write_to_cache)
        m_media_cache.update(sha256_hex, data);
        ...
        std::set<std::string> &result)
{
    ...
    for (u32 pos = 6; pos < data.size(); pos += 32) {
        result.insert(data.substr(pos, 32));
    }
}
...
void SingleMediaDownloader::addFile(const std::string &name, const std::string &sha256)
{
        ...
        const std::string &data, Client *client)
{
    ...
    m_stage = STAGE_DONE;
    checkAndLoad(name, m_file_sha256, data, false, client);
    return true;
}

void SingleMediaDownloader::initialStep(Client *client)
{
    if (tryLoadFromCache(m_file_name, m_file_sha256, client))
        m_stage = STAGE_DONE;
    if (isDone())
        return;
        ...
        const HTTPFetchResult &fetch_result, Client *client)
{
    ...
    if (fetch_result.succeeded) {
        bool success = checkAndLoad(m_file_name, m_file_sha256,
                fetch_result.data, false, client);
        if (success) {
            m_stage = STAGE_DONE;
            return;
...
{
    std::string url = m_remotes.at(m_current_remote) + hex_encode(m_file_sha256);
