...
struct HardcodedPauseFormspecHandler : public TextDest
{
    ...
    void gotText(const StringMap &fields)
    {
        ...
        if (fields.find("btn_mob_spawning_settings") != fields.end()) {
            g_gamecallback->openMobSpawningSettings();
            return;
        }
...
void GameFormSpec::showPauseMenu()
{
    ...
    os    << "button[4," << (ypos++) << ";3,0.5;btn_settings;"
        << strgettext("Settings") << "]";
    os    << "button[4," << (ypos++) << ";3,0.5;btn_mob_spawning_settings;"
        << strgettext("Mob Spawning Settings") << "]";
