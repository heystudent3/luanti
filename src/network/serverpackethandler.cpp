...
void Server::handleCommand_ModChannelMsg(NetworkPacket *pkt)
{
    ...
    *pkt >> channel_name >> channel_msg;
    ...
    // Rate limiting: max 5 messages per second per peer
    float current_time = getTimeMs() / 1000.0f;
    float min_interval = 0.2f; // 200ms between messages (5 per second)
    
    auto rate_it = m_modchannel_rate_limiter.find(peer_id);
    if (rate_it != m_modchannel_rate_limiter.end()) {
        if (current_time - rate_it->second < min_interval) {
            verbosestream << "Rate limiting mod channel message from peer " 
                << peer_id << std::endl;
            return;
        }
        rate_it->second = current_time;
    } else {
        m_modchannel_rate_limiter[peer_id] = current_time;
    }

    // Additional message filtering: check for overly long messages
    if (channel_msg.length() > 1000) {
        warningstream << "Rejecting overly long mod channel message from peer " 
            << peer_id << " (length: " << channel_msg.length() << ")" << std::endl;
        return;
    }
